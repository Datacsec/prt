<template>
  <!-- Sidebar avec design moderne et minimaliste -->
  <aside class="w-64 bg-white shadow-lg flex flex-col h-screen">
    <!-- Logo ou titre en haut -->
    <div class="p-6 border-b border-gray-200">
      <h1 class="text-xl font-bold text-gray-800">Menu</h1>
    </div>

    <!-- Liste des onglets de navigation -->
    <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
      <!-- Item de menu - Default -->
      <router-link
        to="/"
        class="flex items-center px-4 py-3 rounded-lg transition-colors duration-200"
        :class="isActive('/') ? 'bg-primary-100 text-primary-700 font-medium' : 'text-gray-700 hover:bg-gray-100'"
      >
        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
        </svg>
        <span>Accueil</span>
      </router-link>

      <!-- Item de menu - Download -->
      <router-link
        to="/download"
        class="flex items-center px-4 py-3 rounded-lg transition-colors duration-200"
        :class="isActive('/download') ? 'bg-primary-100 text-primary-700 font-medium' : 'text-gray-700 hover:bg-gray-100'"
      >
        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
        </svg>
        <span>Téléchargements</span>
      </router-link>

      <!-- Item de menu - Historique -->
      <router-link
        to="/historique"
        class="flex items-center px-4 py-3 rounded-lg transition-colors duration-200"
        :class="isActive('/historique') ? 'bg-primary-100 text-primary-700 font-medium' : 'text-gray-700 hover:bg-gray-100'"
      >
        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
        <span>Historique</span>
      </router-link>

      <!-- Item de menu - Assurance -->
      <router-link
        to="/assurance"
        class="flex items-center px-4 py-3 rounded-lg transition-colors duration-200"
        :class="isActive('/assurance') ? 'bg-primary-100 text-primary-700 font-medium' : 'text-gray-700 hover:bg-gray-100'"
      >
        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
        </svg>
        <span>Assurance</span>
      </router-link>

      <!-- Item de menu - Banque -->
      <router-link
        to="/banque"
        class="flex items-center px-4 py-3 rounded-lg transition-colors duration-200"
        :class="isActive('/banque') ? 'bg-primary-100 text-primary-700 font-medium' : 'text-gray-700 hover:bg-gray-100'"
      >
        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
        </svg>
        <span>Banque</span>
      </router-link>
    </nav>

    <!-- Section utilisateur en bas avec avatar et logout -->
    <div class="p-4 border-t border-gray-200">
      <!-- Profil utilisateur -->
      <div class="flex items-center px-4 py-3 mb-3">
        <!-- Avatar -->
        <div class="w-10 h-10 rounded-full bg-primary-500 flex items-center justify-center text-white font-semibold mr-3">
          {{ userInitials }}
        </div>
        <!-- Nom de l'utilisateur -->
        <div class="flex-1 min-w-0">
          <p class="text-sm font-medium text-gray-800 truncate">{{ userName }}</p>
          <p class="text-xs text-gray-500 truncate">{{ userEmail }}</p>
        </div>
      </div>

      <!-- Bouton Logout -->
      <button
        @click="handleLogout"
        class="w-full flex items-center justify-center px-4 py-2 text-sm font-medium text-red-600 bg-red-50 rounded-lg hover:bg-red-100 transition-colors duration-200"
      >
        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
        </svg>
        Déconnexion
      </button>
    </div>
  </aside>
</template>

<script setup>
import { computed } from 'vue'
import { useRoute, useRouter } from 'vue-router'

// Router pour la navigation
const route = useRoute()
const router = useRouter()

// Données utilisateur (à remplacer par les vraies données de votre application)
const userName = 'Jean Dupont'
const userEmail = 'jean.dupont@example.com'

// Calcul des initiales pour l'avatar
const userInitials = computed(() => {
  return userName
    .split(' ')
    .map(n => n[0])
    .join('')
    .toUpperCase()
    .slice(0, 2)
})

// Fonction pour vérifier si une route est active
const isActive = (path) => {
  return route.path === path
}

// Fonction de déconnexion
const handleLogout = () => {
  // Ici, vous pouvez ajouter votre logique de déconnexion
  // Par exemple: supprimer le token, rediriger vers la page de connexion, etc.
  alert('Déconnexion effectuée')
  // router.push('/login')
}
</script>
